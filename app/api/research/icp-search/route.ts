...
function buildLushaBody(filters: ParsedFilters, page: number, size: number) {
  const contactInclude: Record<string, unknown> = {}
  const companiesInclude: Record<string, unknown> = {}

  if (filters.jobTitles.length > 0) contactInclude.jobTitles = filters.jobTitles
  ...
  if (filters.locations.length > 0) contactInclude.locations = filters.locations
...
export async function POST(request: NextRequest) {
  ...
  const lushaBody = buildLushaBody(body.filters, page, fetchSize)
